...
\begin{equation}\label{HTV}
  \hat{H}(p,x,t) = T(p) + V(x)
\end{equation}
and use that the commutator $[\epsilon T, \epsilon V]$ is of order $\epsilon^2$ and thus for our small $\epsilon$ can be neglected. This means (we can get this step by using the Baker–Campbell–Hausdorff formula):
\begin{align}\label{UTV}
  \hat{U}(t_{n},t_{n-1}) = e^{\frac{-i \epsilon \hat{H}}{\hbar}} = e^{\frac{-i \epsilon (T + V)}{\hbar}} \\
  = e^{\frac{-i \epsilon \hat{T}}{\hbar}} e^{\frac{-i \epsilon \hat{V}}{\hbar}} (1 + \mathfrac{O} (\epsilon^2) )
\end{align}
